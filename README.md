...
最好的思路肯定是按照上课讲的一遍扫描维护truelist和falselist，可是由于助教给出的文法是没有bool表达式的，甚至与或非都是后来加上去的，所以只能在原来表达式的基础上改，而这样如果还是用原来的方法，会导致所有的表达式都有truelist和falselist，难以维护，所以我采用了一种更简单的方法：`M->空`，规约的时候加一句跳转，而具体的内容需要规约到更上层才能填进去。

例如or的实现：`A or M B`，由于A B的代码都是规约到A B的时候就写好了，所以要想实现A为真就不执行B，就必须提前在A B中间加入一行代码，这个任务就是由`M->空`来完成的。当规约到`A or M B`的时候，在填上M要跳转的地方。

and的实现是一样的思路，不过把M的真跳转变成假跳转

## 3.？.2 非表达式

由于统一采用表达式的形式，所以没什么好说的，就是直接取非就行了。
...
记录一个很离谱的问题：直接这样修改会导致LR(1)分析报错，经过漫长的排查之后发现是LR(1)分析方法在读完 if 表达式 语句块，到达语句块最后的右花括号的时候，无法区分出来这是一个选择语句还是选择语句块，可能这样会导致文法不是LR(1)文法。但是后来发现，只要对选择语句块里面也做类似的修改，也就是在对应位置加上M，它就不会报错了，可能是又变回了LR(1)文法。
...
总体看起来是和IF语句类似的，不过就是最后多了一个跳转回循环开始的位置
...
首先基本的结构，什么时候跳转，这些都很好说。困难的地方在于变量的管理，注意语句块内的变量在规约成语句块的时候就释放掉了，所以这里我们只需要负责释放掉FOR语句声明里面的变量就可以。

以及，记得最后要对FOR语句声明里声明的变量做++操作。

哦对了，本来说不需要j<这类东西了，但是这里还是用了j>=，因为不然的话就要再造一个表达式，不仅麻烦还降低了效率。

## 3.？.7 break语句和continue语句

break和continue的本质是差不多的，都是一句无条件跳转，但是困难的地方在于我们规约出来break或continue语句的时候是不知道具体跳转到哪里的，所以这里要用到和一遍扫描转移类似的思想：对语句串和语句块记录两个字段：breakList和continueList，在规约成循环语句的时候就可以知道具体跳转的位置了：M或者L，然后再遍历这个链表填好就行。
